...
struct screen {
    ...
    GLuint buffer_depthMapFBO{0};
    GLuint texture_depthMap{0};
    int texture_depthMap_Width{1024};
    int texture_depthMap_Height{1024};
...
void Renderer_Init(const int screen_width,
    ...
    glGenFramebuffers(1, &Screen_Texture.buffer_depthMapFBO);
    // our depth map is 1024 * 1024 resolution


    glGenTextures(1, &Screen_Texture.texture_depthMap);
    glBindTexture(GL_TEXTURE_2D, Screen_Texture.texture_depthMap);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT,
                 Screen_Texture.texture_depthMap_Width, Screen_Texture.texture_depthMap_Height,
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, Screen_Texture.buffer_depthMapFBO);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, Screen_Texture.texture_depthMap, 0);
...
void Renderer_FrameEnd() {
    ...
    // for anything that only needs to be drawn
    // just do a plain old pass

    // finally do the last pass
