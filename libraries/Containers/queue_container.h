...
#define QUEUE_IMPLEMENTATION_STATIC(T)                                             \
typedef struct Queue_##T {                          \
int capacity;                                       \
T *data;                                            \
int head, tail, size, mask;                         \
...
static bool Queue_##T##_Resize(Queue_##T *q) {                              \
    ...
    return true;                                                            \
                                                                            ...
                                                                            \
static bool Queue_##T##_Enqueue(Queue_##T *q, T value) {                           \
    if (q->size == q->capacity && !Queue_##T##_Resize(q)) return false;     \
    q->data[q->tail] = value;                                               \
    q->tail = (q->tail + 1) & q->mask;                                      \
    ++q->size;                                                              \
    return true;                                                            \
                                                                            ...
                                                                            \
static Queue_##T *Queue_##T##_Create(int min_capacity) {                           \
    if (min_capacity <= 0) min_capacity = 8; /* reasonable default */       \
                                                                            \
    Queue_##T *q = (Queue_##T *)malloc(sizeof(Queue_##T));                               \
    if (!q) return NULL;                                                    \
    ...
    return q;                                                               \
                                                                            ...
                                                                            \
static bool Queue_##T##_Deque(Queue_##T *q, T *out) {                              \
    if (q->size == 0) return false;                                         \
    *out = q->data[q->head];                                                \
    q->head = (q->head + 1) & q->mask;                                      \
    --q->size;                                                              \
    return true;                                                            \
                                                                            ...
                                                                            \
static void Queue_##T##_Free(Queue_##T *pq) {                                     \
    free(pq->data);                                                      \
    free(pq);                                                              \
}  
